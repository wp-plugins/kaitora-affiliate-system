...
function kdals_friendly_url(){
    ...
    $kdals_junction_word = get_option('kdals_friendly_value');
    if($kdals_junction_word == ""){
        $kdals_junction_word = "out";
    }
    $kdals_friendlier_rule = array($kdals_junction_word.'/([^/\.]+)?$' => 'index.php?kdals_function=recommended_sites&kdals_keyword=$1');
...
function run_kdals($content){
    ...
    preg_match('#([)omit_kdals(])#e',$content,$kdals_active_count);
        ...
        foreach ($kdals_replace_array as $kdals_keyword_go){
            ...
            $kdals_go_url = get_option('home') . '/'.$kdals_junction_word.'/' . $kdals_keyword;    
            } ELSE {
